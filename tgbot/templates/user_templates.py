    ...
    class Settings:
        ...
        class Items:
            class Error:
                def text() -> str:
                    msg = f"⚙️ <b>Настройки бота → 📦 Предметы</b>"\
                            f"\n" \
                            f"\n→ Автоматическое поднятие предметов: <i>не удалось загрузить</i>" \
                            f"\n→ Статус приоритета для поднятых предметов: <i>не удалось загрузить</i>" \
                            f"\n" \
                            f"\n<b>Что такое автоматическое поднятие предметов?</b>" \
                            ...
                            f"только что купили." \
                            f"\n" \
                            f"\n<b>Что такое cтатус приоритета для поднятых предметов?</b>" \
                            f"\nС каким статусом будет выставлять на продажу предмет, который был автоматически поднят. " \
            ...
            class Loading:
                def text() -> str:
                    msg = f"⚙️ <b>Настройки бота → 📦 Предметы</b>"\
                            ...
                            f"\n→ Статус приоритета для поднятых предметов: <i>загрузка</i>" \
                            f"\n" \
                            f"\n<b>Что такое автоматическое поднятие предметов?</b>" \
                            f"\nКак только кто-то купит ваш предмет на Playerok, он сразу же пропадёт из активных " \
                            f"на сайте. Эта функция позволит автоматически \"поднимать на ноги\" предмет, который " \
            ...
            class Default:
                def text() -> str:
                    config = Config.get()
                    auto_raising_items_enabled = "🟢 Включено" if config["auto_raising_items_enabled"] == True else "🔴 Выключено"
                    if config.get("auto_raising_items_priority_status") == "DEFAULT": auto_raising_items_priority_status = "🆓 Бесплатный"
                    elif config.get("auto_raising_items_priority_status") == "PREMIUM": auto_raising_items_priority_status = "⚡ Премиум"
                    else: auto_raising_items_priority_status = "❌ Не задано"
                    msg = f"⚙️ <b>Настройки бота → 📦 Предметы</b>"\
                            f"\n" \
                            f"\n→ Автоматическое поднятие предметов: <code>{auto_raising_items_enabled}</code>" \
                            f"\n→ Статус приоритета для поднятых предметов: <code>{auto_raising_items_priority_status}</code>" \
                            f"\n" \
                            f"\n<b>Что такое автоматическое поднятие предметов?</b>" \
                            f"\nКак только кто-то купит ваш предмет на Playerok, он сразу же пропадёт из активных " \
                            f"на сайте. Эта функция позволит автоматически \"поднимать на ноги\" предмет, который " \
                ...
                def kb() -> InlineKeyboardMarkup:
                    ...
                    btn1 = InlineKeyboardButton(
                        text=f"⬆️ Автоподнятие предметов: {auto_raising_items_enabled}",
                        callback_data="disable_auto_raising_items" if config.get("auto_raising_items_enabled") else "enable_auto_raising_items"
                    )
                    btn2 = InlineKeyboardButton(
                        text=f"⭐ Статус приоритета: {auto_raising_items_priority_status}",
                        callback_data="premium_auto_raising_items_priority_status" if config.get("auto_raising_items_priority_status") == "DEFAULT" else "default_auto_raising_items_priority_status"
                    )
                    btn_refresh = InlineKeyboardButton(
                        text="🔄️ Обновить",
                        callback_data=CallbackDatas.BotSettingsNavigation(
